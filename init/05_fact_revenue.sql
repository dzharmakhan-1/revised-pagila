...
-- Grain: 1 row per payment per category
-- =========================================================

SET search_path TO analytics, public;

-- Drop/recreate ensures clean state when init scripts run
DROP TABLE IF EXISTS analytics.fact_revenue CASCADE;

CREATE TABLE analytics.fact_revenue (
    revenue_id     BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- surrogate PK
...
-- Prevent duplicates for same payment/category/store/geo combo
ALTER TABLE analytics.fact_revenue
ADD CONSTRAINT uq_fact_revenue_grain
UNIQUE (payment_id, category_sk, store_sk, geography_sk);
...
-- Load fact
-- (if this script is ever run manually later, the ON CONFLICT prevents duplicates)
...
-- geography from store address -> city
JOIN public.store s
...
ON CONFLICT (payment_id, category_sk, store_sk, geography_sk) DO NOTHING;
