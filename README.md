...
This project transforms the Pagila OLTP database into a reproducible analytical data warehouse using dimensional modeling, a star schema, and Docker.
...
- Foundations for Slowly Changing Dimensions (SCD)
...
> **One row per payment, per category, per store, per geography, per date**
...
- Enable Slowly Changing Dimension (SCD) handling
- Follow dimensional modeling best practices

### SCD Notes (Type 2)
The dimensions are modeled with surrogate keys and natural keys so they can be extended to SCD Type 2 if source attributes change over time.
This repo currently loads a single snapshot, but the schema is ready for:
- Adding `effective_date`, `end_date`, `is_current`
- Tracking changes in store or geography attributes without breaking fact history
...
5. **Create views**: Business-ready views for common rollups and slicing.

The process is SQL-first and deterministic, so a fresh container always produces the same analytical state.
...
- The difference between writing queries and designing analytical systems
...
![Example Query 1](screenshots/example_query3.png)
...
![Example Query 3](screenshots/example_query2.png)
...
- Dimension tables provide descriptive context:
  - `dim_date` for time
  - `dim_category` for film category
  - `dim_store` and `dim_geography` for location
...
- Simple joins (fact plus dimensions)
...
- Use **pgAdmin4** for screenshots (professional and clean output)
- Use **PowerShell / psql** for smoke tests and automation
- README screenshots should come from **pgAdmin4** (or another GUI SQL client) for readability and portfolio quality

At this point, the project is no longer just SQL plus Docker.  
It is a **complete analytical data warehouse with business storytelling.**
